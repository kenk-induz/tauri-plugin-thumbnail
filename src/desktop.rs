...
impl<R: Runtime> Thumbnail<R> {
  ...
  pub fn get_thumbnail(&self, payload: GetThumbnailRequest) -> crate::Result<GetThumbnailResponse> {
    let path = Path::new(&payload.path);
    if !path.exists() {
        return Err(crate::Error::Io(std::io::Error::new(std::io::ErrorKind::NotFound, "File not found")));
    }
    ...
    #[cfg(target_os = "linux")]
    {
        if let Ok(thumb_data) = self.get_linux_thumbnail(&payload.path) {
            println!("Getting thumbnail from linux native");
  ...
  fn get_linux_thumbnail(&self, file_path: &str) -> crate::Result<Vec<u8>> {
    ...
    let cache_dir = std::env::var("XDG_CACHE_HOME")
        ...
        .unwrap_or_else(|_| {
            home_dir().map(|h| h.join(".cache")).unwrap_or_default()
        });
  ...
  fn get_windows_thumbnail(&self, file_path: &str) -> crate::Result<Vec<u8>> {
    ...
    unsafe {
        let item = SHCreateItemFromParsingName(file_path, None, &windows::Win32::UI::Shell::IFileExtractIcon::IID).map_err(|_| crate::Error::NotFound)?;
        let extract_icon: windows::Win32::UI::Shell::IFileExtractIcon = std::mem::transmute_copy(&item);
    ...
    Err(crate::Error::UnsupportedType("Windows thumbnail extraction requires COM implementation".to_string()))
  }
  ...
  fn get_macos_thumbnail(&self, file_path: &str) -> crate::Result<Vec<u8>> {
    ...
    // Generate preview using sips on macOS
    // sips -s format png -o <output> <input>
    let output = Command::new("sips")
        .args([
            "-s", "format", "png",
            "-o", &format!("/tmp/thumb_{}.png", hash_path(&Path::new(file_path))),
            file_path,
        ])
        .output();
    ...
    if let Ok(out) = output {
        if out.status.success() {
            let thumb_path = format!("/tmp/thumb_{}.png", hash_path(&Path::new(file_path)));
            if let Ok(data) = fs::read(&thumb_path) {
                let _ = fs::remove_file(thumb_path);
                return Ok(GetThumbnailResponse {
                    thumbnail: data,
